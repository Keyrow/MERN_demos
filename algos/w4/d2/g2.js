// Group Members: Chris, David, Faisal
...
function lense(user, keys) {
  // check if user object has property of key
  if (user.hasownproperty(keys[0])) {
    if (user[keys[0]].hasownproperty(keys[1])) {
      if (user[keys[0]][keys[1]].hasownproperty(keys[2])) {
        return user[keys[0][keys[1]][keys[2]]];
      }
    }
  }
}
